...
def matrix_constraint(
    ...
    if cmds.nodeType(constrain_transform) == "joint":
        ...
        orient_mult_matrix: str = cmds.createNode("multMatrix", name=f"{constrain_transform}_ConstraintOrientMatrix")
        ...
        # Next we multiply by the world matrix of the source transform
        cmds.connectAttr(f"{source_transform}.worldMatrix[0]", f"{orient_mult_matrix}.matrixIn[{orient_mult_index}]")
        orient_mult_index += 1
        ...
        if use_parent:
            ...
            cmds.connectAttr(f"{orient_offset_node}.outputMatrix", f"{orient_mult_matrix}.matrixIn[{orient_mult_index}]")
            orient_mult_index += 1
        else:
            ...
            cmds.setAttr(f"{orient_mult_matrix}.matrixIn[{orient_mult_index}]", inverse_orient_matrix, type="matrix")
            orient_mult_index += 1
            # Cleanup temp node
        ...
        orient_decompose_matrix: str = cmds.createNode("decomposeMatrix", name=f"{constrain_transform}_ConstrainOrientDecompose")
    ...
    # Hook up the translate and scale as well    
