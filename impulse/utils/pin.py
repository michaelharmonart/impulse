...
def make_matrix_pin(
    ...
    if normalize:
        ...
        cmds.connectAttr(f"{shape_origin}.minValueU", f"{u_min.input}[0]")
        cmds.setAttr(f"{u_min.input}[1]", margin)
        u_max = node.SumNode(name=f"{u_clamp}_max")
        cmds.connectAttr(f"{shape_origin}.maxValueU", f"{u_max.input}[0]")
        cmds.setAttr(f"{u_max.input}[1]", -margin)

        v_min = node.SumNode(name=f"{v_clamp}_min")
        cmds.connectAttr(f"{shape_origin}.minValueV", f"{v_min.input}[0]")
        cmds.setAttr(f"{v_min.input}[1]", margin)
        v_max = node.SumNode(name=f"{v_clamp}_max")
        cmds.connectAttr(f"{shape_origin}.maxValueV", f"{v_max.input}[0]")
        cmds.setAttr(f"{v_max.input}[1]", -margin)
    ...
    if volume_preservation:
        ...
        norm_volume_node.connect_input(input_attr=cross_volume.output, input_number=1)
        norm_volume_node.connect_input(input_attr=orig_cross_length.output, input_number=2)
