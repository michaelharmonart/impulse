...
from impulse.utils.spline.math import point_on_spline_weights


def add_colors(
    color1: tuple[float, float, float], color2: tuple[float, float, float]
) -> tuple[float, float, float]:
    return tuple(channel1 + channel2 for channel1, channel2 in zip(color1, color2))


def scale_color(color: tuple[float, float, float], scalar: float) -> tuple[float, float, float]:
    return tuple(channel * scalar for channel in color)


def blend_colors_by_weight(colors: Sequence[tuple[float, float, float]], weights: Sequence[float]):
...
def sample_spline_gradient(gradient: Gradient, position: float) -> tuple[float, float, float]:
    ...
    gradient_stop_positions = [stop.position for stop in gradient.stops]
